...
export class ActivityService extends HttpService {
  ...
  public getById(activityId: number | string): Observable<IActivity> {
    return this.http.get<IActivity>(`${this.prefix}/getById/${activityId}`).pipe(
      switchMap((activity) => {
        return of({
          ...activity,
          ...
          endDate: formatActivitiesTime(activity.endDate, this.env.timeZone),
        });
      })
    );
  }
  ...
  public getByEventId(eventId: number | string): Observable<Schedule> {
    return this.http.get<Schedule>(`${this.prefix}/getByEventId/${eventId}`).pipe(
      switchMap((schedule) => {
        return of(
...
const formatActivitiesTime = (dateTime: string | Date, timeZone: number = 0) =>
